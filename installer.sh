...
echo "5. CoolRune-NVIDIA-DESKTOP-OPENSOURCE"
echo "6. CoolRune-NVIDIA-DESKTOP-PROPRIETARY"
...
# COOLRUNE INSTALL
7z x coolrune-dotfiles-laptop.7z -o/home/$USER/ -y && 7z x coolrune-root-laptop.7z -o/ -y && s6-service add default apparmor && s6-service add default fail2ban && s6-service add default NetworkManager && s6-service add default dnscrypt-proxy && s6-service add default ufw && s6-service add default cpupower && rm /etc/s6/adminsv/default/contents.d/connmand && pacman -Rdd --noconfirm vlc-luajit connman connman-s6 connman-gtk && s6-db-reload && grub-mkconfig -o /boot/grub/grub.cfg &&
...
### NVIDIA DESKTOP OPENSOURCE CHOICE ###
elif [ "$choice" = "5" ]; then
...
### NVIDIA DESKTOP PROPRIETARY CHOICE ###
elif [ "$choice" = "6" ]; then
